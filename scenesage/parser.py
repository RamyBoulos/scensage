...
def segment_into_scenes(subs: List[Dict], min_gap_seconds: int = 4, mode: str = "llm", llm_segmenter=None) -> List[Dict]:
    """
    Groups subtitle entries into scenes using either:
    - 'gap' mode: based on a time gap ≥ min_gap_seconds
    - 'llm' mode: uses an LLM-based segmenter function (if provided)
    """
    if mode == "llm" and llm_segmenter is not None:
        return llm_segmenter(subs)
